...
export default function ColaboradoresModalButton({ colaboradores }: AlunoModalButtonProps) {
  ...
  const filteredColaboradores = colaboradores?.filter((colaborador) => colaborador.nome.toLowerCase().includes(searchTerm.toLowerCase()));

  const indexOfLastColaborador = currentPage * resultsPerPage;
  const indexOfFirstColaborador = indexOfLastColaborador - resultsPerPage;
  const currentAlunos = filteredColaboradores?.slice(indexOfFirstColaborador, indexOfLastColaborador);

  const totalPages = Math.ceil((filteredColaboradores?.length || 0) / resultsPerPage);
        ...
        <div className="py-4">
          {currentAlunos && currentAlunos.length > 0 ? (
            <table className="table w-full">
              ...
              <tbody>
                {currentAlunos.map((colaborador) => (
                  <tr key={colaborador.id}>
                    <td>{colaborador.nome}</td>
                    <td>{colaborador.email.toLowerCase()}</td>
                    <td>
                      <Link className="btn btn-outline btn-sm" onClick={closeModal} href={`/help/admin/factory/edit/colaborador/${colaborador.id}`}>
                        <BiEdit />
                      </Link>
                    </td>
                  </tr>
